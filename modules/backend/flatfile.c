		...
		else if (!strcmp("CA", item))
		{
			...
			if (cachan && causer)
			{
				...
				if (versn >= DB_ATHEME)
				{
					...
					if (their_ca_all == 0)
					{
						their_ca_all = CA_VOICE | CA_AUTOVOICE | CA_OP | CA_AUTOOP | CA_TOPIC | CA_SET | CA_REMOVE | CA_INVITE | CA_RECOVER | CA_FLAGS | CA_HALFOP | CA_AUTOHALFOP | CA_ACLVIEW | CA_AKICK | CA_SUSPENDED;
