...
static void do_list(sourceinfo_t *si, mychan_t *mc, unsigned int flags)
{
	...
	MOWGLI_ITER_FOREACH(n, mc->chanacs.head)
	{
		...
		if (template != NULL)
			command_success_nodata(si, _("%-5d %-22s %-20s (%s) (%s) [modified %s ago, by %s]"),
				i, ca->entity ? ca->entity->name : ca->host, bitmask_to_flags(ca->level), template, mc->name, mod_ago, setter_name);
		else
			command_success_nodata(si, _("%-5d %-22s %-20s (%s) [modified %s ago, by %s]"),
				i, ca->entity ? ca->entity->name : ca->host, bitmask_to_flags(ca->level), mc->name, mod_ago, setter_name);
		i++;
	}
