 ...
 *
 * This file contains protocol support for foxaircd, a charybdis fork.
...
#include "uplink.h"
#include "pmodule.h"
#include "protocol/foxaircd.h"

DECLARE_MODULE_V1("protocol/foxaircd", true, _modinit, NULL, PACKAGE_STRING, "ShaleCoding IRC Network Development Team <http://shale.chatfreedom.us>");
...
struct extmode foxaircd_ignore_mode_list[] = {
  { 'f', check_forward },
...
static mowgli_node_t *foxaircd_next_matching_ban(channel_t *c, user_t *u, int type, mowgli_node_t *first)
{
	chanban_t *cb;
	mowgli_node_t *n;
...
static bool foxaircd_is_valid_host(const char *host)
{
	return true;
}

static void foxaircd_notice_channel_sts(user_t *from, channel_t *target, const char *text)
{
	sts(":%s NOTICE %s :%s", from ? CLIENT_NAME(from) : ME, target->name, text);
}

static bool foxaircd_is_extban(const char *mask)
{
...
void _modinit(module_t * m)
{
	...
	notice_channel_sts = &foxaircd_notice_channel_sts;

	next_matching_ban = &foxaircd_next_matching_ban;
	is_valid_host = &foxaircd_is_valid_host;
	is_extban = &foxaircd_is_extban;

	mode_list = foxaircd_mode_list;
	ignore_mode_list = foxaircd_ignore_mode_list;
	...
	ignore_mode_list_size = ARRAY_SIZE(foxaircd_ignore_mode_list);
