 ...
 *
 * This file contains protocol support for chamberircd, a charybdis fork.
...
#include "uplink.h"
#include "pmodule.h"
#include "protocol/chamberircd.h"

DECLARE_MODULE_V1("protocol/chamberircd", true, _modinit, NULL, PACKAGE_STRING, "ShaleCoding IRC Network Development Team <http://shale.chatfreedom.us>");

/* *INDENT-OFF* */

ircd_t chamberircd = {
        "chamberircd",			/* IRCd name */
	...
	PROTOCOL_CHAMBERIRCD,		/* Protocol type */
...
struct cmode_ chamberircd_mode_list[] = {
  { 'i', CMODE_INVITE },
  { 'm', CMODE_MOD    },
  { 'n', CMODE_NOEXT  },
  { 'p', CMODE_PRIV   },
...
struct extmode chamberircd_ignore_mode_list[] = {
  { 'f', check_forward },
  { 'j', check_jointhrottle },
  { '\0', 0 }
};

struct cmode_ chamberircd_status_mode_list[] = {
  { 'q', CSTATUS_OWNER },
  { 'a', CSTATUS_PROTECT },
  { 'o', CSTATUS_OP    },
  { 'h', CSTATUS_HALFOP },
...
struct cmode_ chamberircd_prefix_mode_list[] = {
  { '~', CSTATUS_OWNER },
  { '&', CSTATUS_PROTECT },
  { '@', CSTATUS_OP    },
  { '%', CSTATUS_HALFOP },
...
struct cmode_ chamberircd_user_mode_list[] = {
  { 'p', UF_IMMUNE   },
  { 'a', UF_ADMIN    },
  { 'i', UF_INVIS    },
  { 'o', UF_IRCOP    },
...
static mowgli_node_t *chamberircd_next_matching_ban(channel_t *c, user_t *u, int type, mowgli_node_t *first)
{
	chanban_t *cb;
	mowgli_node_t *n;
...
static bool chamberircd_is_valid_host(const char *host)
{
	return true;
}

static void chamberircd_notice_channel_sts(user_t *from, channel_t *target, const char *text)
{
	sts(":%s NOTICE %s :%s", from ? CLIENT_NAME(from) : ME, target->name, text);
}

static bool chamberircd_is_extban(const char *mask)
{
...
void _modinit(module_t * m)
{
	...
	notice_channel_sts = &chamberircd_notice_channel_sts;

	next_matching_ban = &chamberircd_next_matching_ban;
	is_valid_host = &chamberircd_is_valid_host;
	is_extban = &chamberircd_is_extban;

	mode_list = chamberircd_mode_list;
	ignore_mode_list = chamberircd_ignore_mode_list;
	status_mode_list = chamberircd_status_mode_list;
	prefix_mode_list = chamberircd_prefix_mode_list;
	user_mode_list = chamberircd_user_mode_list;
	ignore_mode_list_size = ARRAY_SIZE(chamberircd_ignore_mode_list);
